...
asset::SAssetBundle CImageLoaderJPG::loadAsset(io::IReadFile* _file, const asset::IAssetLoader::SAssetLoadParams& _params, asset::IAssetLoader::IAssetLoaderOverride* _override, uint32_t _hierarchyLevel)
{
	...
	{
		os::Printer::log("Can't load libjpeg threw an error:", _file->getFileName(), ELL_ERROR);
	...
	switch (cinfo.jpeg_color_space)
	{
		...
		case JCS_YCbCr:
			...
			break;
		case JCS_CMYK:
			os::Printer::log("CMYK color space is unsupported:", _file->getFileName(), ELL_ERROR);
			return {};
			break;
		case JCS_YCCK: // this I have no resources on
			os::Printer::log("YCCK color space is unsupported:", _file->getFileName(), ELL_ERROR);
			return {};
			break;
		case JCS_BG_RGB: // interesting
			os::Printer::log("Loading JPEG Big Gamut RGB is not implemented yet:", _file->getFileName(), ELL_ERROR);
			return {};
			break;
		case JCS_BG_YCC: // interesting
			...
			break;
		default:
			os::Printer::log("Can't load as color space is unknown:", _file->getFileName(), ELL_ERROR);
			return {};
			break;
	}
